...
### Phase 4: cases → features 마이그레이션 ✅ (완료 - Archive 전략)

**목표**: 기존 `docs/cases/` 문서를 Archive하고 새 구조로 전환

**전략 변경**:
- ❌ 전체 이동 (복잡도 높음, 불필요)
- ✅ Archive 보존 + 새 구조 시작 (Clean Slate)

**작업 내용**:
- [x] `docs/cases/` → `docs/cases-archive/` 이동 (git mv)
- [x] `docs/README.md` 생성 (네비게이션 가이드)
- [x] `runs/{run-id}/` 구조 도입 (Phase 3에서 예시 생성 완료)
- [x] 새 구조 명시: `services/{service-name}/docs/features/{feature-name}/`
- [x] 검증: Archive 접근성 확인
- [ ] Publish 프로세스 구현 (향후 과제)

**실제 소요**: 약 15분
**실제 영향**: Low (Archive만 이동, 새 feature부터 Plan05 적용)
...
- **기존 문서 참조**: `docs/cases-archive/` (읽기 전용)
- **Publish 프로세스**: runs/ → output/ 발행 자동화 (향후)
...
- 작업 완료일: 2026-01-11
- 변경 사항:
  - `docs/cases/` → `docs/cases-archive/` 이동
  - `docs/README.md` 생성 (네비게이션 가이드)
