...
import { SkillLoader, getDefaultSkillLoader } from './tool-loader.js';
 ...
 */
export const LegacyToolTypeMap = {
  'query-agent': 'query',
  'code-agent': 'coder',
...
export class ToolRegistry {
  ...
  constructor(config = {}) {
    ...
    this.toolsRoot = path.join(__dirname);
  ...
  async loadAll(skillTypes = null) {
    const targetSkills = skillTypes || Object.values(ToolType);
    const results = {
      success: [],
      failed: [],
  ...
  async loadSkill(skillType) {
    ...
    try {
      ...
      let AgentClass = null;
      try {
        const indexPath = path.join(this.toolsRoot, skillType, 'index.js');
 ...
 */
export function getToolRegistry(config = {}) {
  if (!instance) {
...
export const SkillType = ToolType;
export const getSkillRegistry = getToolRegistry;
