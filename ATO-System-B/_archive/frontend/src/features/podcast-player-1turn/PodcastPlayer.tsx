...
export function PodcastPlayer() {
  ...
  function getStatusMessage() {
    switch (status) {
      case 'idle':
        return '재생 버튼을 눌러 시작하세요';
      case 'playing':
        return `재생 중... ${currentLineIndex + 1} / ${script.length}`;
      case 'paused':
        return `일시정지 ${currentLineIndex + 1} / ${script.length}`;
      case 'completed':
        return '재생 완료';
      default:
        return '';
