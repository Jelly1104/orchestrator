/**
 * BaseTool - 모든 Tool의 추상 부모 클래스
 *
 * Tool 모듈의 표준 인터페이스를 정의하여 일관성 있는 구현을 보장합니다.
 *
 * @version 2.0.0
 * @since 2025-12-29
...
export class BaseTool {
   ...
   * @param {string} config.name - 툴 이름 (필수)
   * @param {string} config.version - 툴 버전 (필수)
  ...
  constructor(config = {}) {
    ...
    if (new.target === BaseTool) {
      throw new Error('BaseTool은 추상 클래스입니다. 직접 인스턴스화할 수 없습니다.');
    }

    // 필수 속성 검증
    if (!config.name) {
      throw new Error('BaseTool: name은 필수입니다.');
    }
    if (!config.version) {
      throw new Error('BaseTool: version은 필수입니다.');
    }
   ...
   *
   * @param {string} toolDir - 툴 디렉토리 경로
   * @returns {Promise<this>}
   */
  async initialize(toolDir) {
    if (toolDir) {
      this.toolLoader = new ToolLoader(toolDir);
      this.tool = await this.toolLoader.loadTool(this.name);
    }
    this.debug(`Initialized v${this.version}`);
    return this;
  }
