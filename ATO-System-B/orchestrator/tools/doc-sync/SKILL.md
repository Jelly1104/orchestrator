...
> **변경 이력**: 네이밍 리팩토링 - Doc-Agent → Doc-Sync Skill
...
```bash
node orchestrator/skills/doc-sync/sync.js --to-notion CLAUDE.md
node orchestrator/skills/doc-sync/sync.js --from-notion all
```
...
```
orchestrator/skills/doc-sync/
├── index.js           # 메인 Skill 클래스
├── sync.js            # 동기화 핵심 로직
...
```javascript
// Orchestrator에서 DocSyncSkill 호출
import { getDocSyncSkill } from './skills/doc-sync/index.js';

const docSyncSkill = getDocSyncSkill();

// 동기화 상태 확인
const status = await docSyncSkill.getStatus();

// Notion으로 동기화 준비
const result = await docSyncSkill.prepareToNotion('CLAUDE.md');
