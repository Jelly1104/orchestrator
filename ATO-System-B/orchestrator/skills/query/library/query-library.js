...
export class QueryLibrary {
  ...
  async initialize() {
    ...
    console.log(`[QueryLibrary] 템플릿 로드 중: ${this.libraryPath}`);
    ...
    for (const [key, config] of Object.entries(TEMPLATE_MAPPINGS)) {
      ...
      try {
        if (fs.existsSync(filePath)) {
          ...
          console.log(`  ✅ ${config.file}: ${queries.length}개 쿼리 로드됨`);
        } else {
          console.warn(`  ⚠️ ${config.file}: 파일 없음`);
        }
      } catch (error) {
    ...
    this.loaded = true;
    console.log(`[QueryLibrary] 총 ${this.templates.size}개 템플릿 로드 완료`);
  }
  ...
  findMatchingTemplate(query) {
    if (!this.loaded) {
      console.warn('[QueryLibrary] 초기화되지 않음, 먼저 initialize() 호출 필요');
      return null;
    }
    ...
    if (bestMatch && bestMatch.score >= MIN_SCORE) {
      console.log(`[QueryLibrary] ✅ 템플릿 매칭: ${bestMatch.key} (score: ${bestMatch.score})`);
      return bestMatch;
    }

    console.log(`[QueryLibrary] ❌ 매칭 템플릿 없음 (max score: ${maxScore})`);
    return null;
  }
  ...
  loadQueries(templateKey, params = {}, queryIndices = null) {
    ...
    if (!template) {
      console.warn(`[QueryLibrary] 템플릿 없음: ${templateKey}`);
      return [];
    }
    ...
    console.log(`[QueryLibrary] ${templateKey}에서 ${processedQueries.length}개 쿼리 로드 [Source: Library]`);
