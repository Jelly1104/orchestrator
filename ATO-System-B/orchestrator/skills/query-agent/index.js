/**
 * QueryAgent - SQL 쿼리 생성 및 데이터 분석 전담
 *
 * AnalysisAgent의 쿼리 생성 기능을 활용하는 전문화된 Agent
 *
 * @version 1.0.0
 * @since 2025-12-19
 */
...
export class QueryAgent {
   ...
   */
  async initialize() {
    this.skill = await this.skillLoader.loadSkill('query-agent');
    console.log('[QueryAgent] Initialized with SKILL.md');
    return this;
  }
  ...
  async analyze(input) {
    ...
    console.log('[QueryAgent] Starting analysis');
    console.log(`  Goals: ${analysisGoals.length} items`);
    console.log(`  Segment: ${targetSegment}`);
    console.log(`  TimeRange: ${timeRange?.start} ~ ${timeRange?.end}`);
    ...
    try {
      ...
      // 결과를 QueryAgent 출력 형식으로 변환
      return this._formatOutput(result, input);
    } catch (error) {
      console.error(`[QueryAgent] Analysis failed: ${error.message}`);
      throw error;
    }
  }
  ...
  async executeQuery(sql, goal = 'Custom Query') {
    console.log(`[QueryAgent] Executing: ${goal}`);
