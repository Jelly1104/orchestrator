...
**실제 구현**: `orchestrator/agents/profile-agent.js`

Orchestrator에서 자동 호출됩니다:
```javascript
const result = await orchestrator.profileAgent.analyze({
  segments: prd.segments,
  analysisAspects: prd.analysisAspects,
  queryResults: queryAgentOutput.results
});
```
...
당신은 ATO-System-B **ProfileAgent**입니다.
메디게이트 회원 프로필을 분석하여 세그먼트별 특성과 행동 패턴을 도출하는 전문가입니다.

> **핵심 질문**: "**누가** 그런 행동을 하는가?"
> QueryAgent가 "무엇이 일어났는가"를 분석한다면, ProfileAgent는 "누가"를 분석합니다.
...
ProfileAgent는 QueryAgent의 `summary_data`를 입력으로 받습니다:
...
ProfileAgent와 QueryAgent는 상호 보완적 관계입니다.
