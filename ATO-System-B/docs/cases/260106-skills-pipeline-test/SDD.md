# SDD.md - Skill Dashboard

> **문서 버전**: 1.0.0
> **최종 업데이트**: 2026-01-06
> **관련 PRD**: .claude/project/PRD.md (Case ID: 260106-skills-pipeline-test)
...
| 버전 | 1.0.0 |
| 작성일 | 2026-01-06 |
| 관련 HANDOFF | docs/cases/260106-skills-pipeline-test/HANDOFF.md |

### 1.2 기능 요약

ATO 시스템의 7개 Skill(leader, designer, coder, reviewer, imleader, query, profiler)을
카드 형식으로 표시하는 React 대시보드. 각 Skill의 이름, 버전, 상태(active/inactive)를 시각화한다.
...
| 컴포넌트 | Props | 설명 |
|----------|-------|------|
| SkillsDashboard | - | 메인 컨테이너 (Header + Grid + Footer) |
| SkillCard | Skill | 개별 Skill 카드 (inline 또는 분리) |

### 4.4 SkillsDashboard 구조

```typescript
// SkillsDashboard.tsx 구조
function SkillsDashboard() {
  // 1. 데이터: SKILLS_DATA 사용
  // 2. 통계 계산: activeCount, inactiveCount
  // 3. 렌더링:
  //    - Header (제목 + Refresh 버튼)
  //    - Grid (SkillCard x7)
  //    - Footer (통계 요약)
}
```
...
import { SkillsDashboard } from './features/skills-dashboard'  // <- import 추가
...
| 파일 | 역할 |
|------|------|
| `frontend/src/features/skills-dashboard/index.ts` | 배럴 export |
| `frontend/src/features/skills-dashboard/types.ts` | 타입 정의 |
| `frontend/src/features/skills-dashboard/SkillsDashboard.tsx` | 메인 컴포넌트 |
