...
> **⚠️ Orchestrator vs Extension 모드**: 이 문서의 다이어그램은 **Orchestrator 기준**으로 작성되어 있습니다. Orchestrator 모드에서는 각 Phase 완료 후 ImpLeader → HITL이 진행되고, 파이프라인 최종 완료 시 ImpLeader → Leader 보고 → HITL로 마무리됩니다. **Extension 모드**(`.claude/skills/` 사용)에서는 HITL이 파이프라인 최종 완료 후 한 번만 진행됩니다. Extension Skill 순서는 `.claude/skills/README.md`를 참조하세요.
...
graph TD
    ...
    PhaseCheck -->|No| LeaderReport[🧠 Leader 보고]
    LeaderReport --> FinalHITL[📢 Final HITL]
    FinalHITL -->|Approved| DocSync
    ...
    style FinalHITL fill:#f96,stroke:#333,color:white
    style LeaderReport fill:#f9f,stroke:#333
...
> **원본**: ROLE_ARCHITECTURE.md의 **파이프라인 타입** 및 **HITL 체크포인트** 섹션 참조

```
┌─────────────────────────────────────────────────────────────────┐
│ Phase 0: PRD Gap Check - 🧠 Leader                              │
│   - 필수 항목 체크 (목적, 타겟, 기능, 지표)                        │
│   - 파이프라인 타입 결정 → HANDOFF 생성                           │
│   - 누락 시 HITL (사용자 보완 요청)                                │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ Pipeline Router (6개 타입)                                       │
│   analysis(A) | design(B) | code(C)                              │
│   analyzed_design(A→B) | ui_mockup(B→C) | full(A→B→C)           │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ Phase A: Analysis - 🕵️ Analyzer                                 │
│   - SQL 실행, 분석 리포트 생성                                     │
│   → 👮 ImpLeader 검증 → PASS: HITL → 다음 Phase                   │
│                       → FAIL: HITL (예외승인/규칙수정/재작업)       │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
│   - HANDOFF+SDD 기반 코드 구현                                    │
│   - 동적 검증 (빌드/구동 테스트)                                   │
│   → 👮 ImpLeader 검증 → PASS: Phase F로                           │
│                       → FAIL: HITL (예외승인/규칙수정/재작업)       │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ Phase F: Final Review - 🧠 Leader                                │
│   - PRD 대비 산출물 검토                                          │
│   - HITL 최종 승인                                                │
│   - DocSync (Notion 동기화 - 계획)                                │
└─────────────────────────────────────────────────────────────────┘
```

> **검증 상세**: VALIDATION_GUIDE.md 참조
...
│   │   └── SEGMENT_RULES.md
│   └── prd/               # PRD 템플릿
│       ├── PRD_LITE.md    # Fast-track 실행용 (핵심 가설 + 즉시 구현)
│       └── PRD_FULL.md    # Deep-dive 실행용 (전 과정 분석 + 정밀 검증)
│
├── skills/                # [Group E] Extension용 경량 Skills
│   ├── leader/SKILL.md    # PRD 분석, 파이프라인 결정 (/leader)
│   ├── profiler/SKILL.md  # 세그먼트 정의 (/profiler)
│   ├── query/SKILL.md     # SQL 쿼리 생성 (/query)
│   ├── designer/SKILL.md  # 설계 문서 생성 (/designer)
│   ├── coder/SKILL.md     # 코드 구현 (/coder)
│   └── imleader/SKILL.md  # 산출물 검증 - HANDOFF 기준 (/imleader)
│
...
graph TD
    ...
    IMP_A2 -- Fail --> ANA_AB
    IMP_A2 -- Pass --> HITL_A_AB[📢 HITL]
    HITL_A_AB -->|Approved| DSG_AB[📐 Designer]
    ...
    IMP_B3 -- Fail --> DSG_BC
    IMP_B3 -- Pass --> HITL_B_BC[📢 HITL]
    HITL_B_BC -->|Approved| CODE_BC[💻 Coder]
    ...
    IMP_A3 -- Pass --> HITL_A_Full[📢 HITL]
    HITL_A_Full -->|Approved| DSG_Full[📐 Designer]
    DSG_Full -->|IA/WF/SDD| IMP_B4[👮 ImpLeader]
    IMP_B4 -- Fail --> DSG_Full
    IMP_B4 -- Pass --> HITL_B_Full[📢 HITL]
    HITL_B_Full -->|Approved| CODE_Full[💻 Coder]
    ...
    style HITL_A_AB fill:#f96,stroke:#333,color:white
    style HITL_AB fill:#f96,stroke:#333,color:white
    style HITL_B_BC fill:#f96,stroke:#333,color:white
    style HITL_BC fill:#f96,stroke:#333,color:white
    style HITL_A_Full fill:#f96,stroke:#333,color:white
    style HITL_B_Full fill:#f96,stroke:#333,color:white
