...
> **구현 참조**: 금지 패턴 정규식 및 검증 코드는 `DB_ACCESS_POLICY.md`의 **쿼리 검증** 섹션을 참조하세요.
...
| 문서 | 역할 | 참조 시점 |
| ---- | ---- | --------- |
| `DOMAIN_SCHEMA.md` | 스키마 정합성 검증 기준, 민감 데이터 정의 | 스키마 검증 단계 |
| `DB_ACCESS_POLICY.md` | 금지 패턴 정규식, 검증 코드 구현 | 구현 참조 |
| `PRD_GUIDE.md` | PRD 유형/파이프라인 정의 | PRD Gap Check 단계 |
| `CLAUDE.md` | 품질 기준 최상위 근거 | 전체 |

---

## 8. 보안 게이트 (Security Gates) 🔒

> **v2.1.0 추가**: CLAUDE.md에서 이관 (레드팀 분석 결과 반영)

### 8.1 입력 검증 (Input Validation)
