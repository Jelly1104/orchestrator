...
>
> **산출물 저장 위치**: DOCUMENT_PIPELINE.md - **파이프라인 타입별 산출물** 섹션 참조
...
| 함수 설계 | 단일 책임 원칙 (SRP) 준수 |
| 타입 안정성 | 타입 회피 금지 (any, dynamic 등) |

### 테스트 (공통 원칙)

| 항목 | 기준 |
|------|------|
| 테스트 필수 | 주요 함수/컴포넌트에 테스트 코드 포함 |
| 테스트 독립성 | 테스트 간 의존성 없음 |
| 테스트 결과 | 모든 테스트 PASS |

> 커버리지 임계값은 `PROJECT_STACK.md`에서 정의.
...
| 항목 | 기준 | 심각도 |
|------|------|--------|
| SQL Injection | 파라미터 바인딩 필수 | 🚨 ERROR |
| XSS 방지 | 사용자 입력 이스케이프 | 🚨 ERROR |
| 민감정보 보호 | PII 평문 노출 금지 | 🚨 ERROR |
| 로그 마스킹 | 민감정보 로그 금지 | 🚨 ERROR |
...
|------|------|
| 빌드 테스트 | 컴파일 성공 (에러 0개) |
| 엔트리포인트 | 진입점에서 컴포넌트/모듈 연결됨 |
| 구동 테스트 | 실행 시 크리티컬 에러 없음 |

> 빌드 명령어, 엔트리포인트 경로는 `PROJECT_STACK.md` 또는 `SDD.md` 참조.
...
| 항목 | 기준 | 심각도 |
|------|------|--------|
| 룰북 불변성 | `.claude/rules/`, `CLAUDE.md` 변경 금지 | 🚨 ERROR |
| DB 안전성 | INSERT/UPDATE/DELETE 쿼리 금지 (Phase C) | 🚨 ERROR |
