...
<!-- [추가] PRDAnalyzer가 이 문서를 로딩하도록 명시 -->
...
<!-- [기존 유지] PRD_TEMPLATE_V2.md 섹션 "필수 정보" 내용 -->
...
<!-- [핵심] PRD_TYPE_PIPELINE.md의 유형 분류 기준 통합 -->
...
<!-- [핵심] PRD_TYPE_PIPELINE.md 섹션 1 통합. PRDAnalyzer가 이 기준으로 판별 -->
...
<!--
[핵심 변경]
기존: prd-analyzer.js에 하드코딩
TO-BE: 이 문서에서 동적으로 추출
-->
...
<!-- [기존 유지] PRD_TYPE_PIPELINE.md 섹션 2 통합 -->
...
<!-- [핵심] PRD_REFERENCE_MAP.md 전체 내용 통합 -->
...
<!-- [기존 유지] PRD_TEMPLATE_V2.md 산출물 타입 표 -->
...
| `OUTPUT_VALIDATION.md` | 산출물 검증 기준                        |
| `PRD_GAP_CHECK.md`     | Gap Check 프로세스                      |
...
[통합] PRD_TEMPLATE_V2.md + PRD_TYPE_PIPELINE.md + PRD_REFERENCE_MAP.md → 단일 문서
- 이유: 3개 문서가 분리되어 PRDAnalyzer가 어떤 문서도 로딩하지 않았음
- 효과: PRDAnalyzer가 단일 문서만 로딩하면 모든 PRD 관련 정보 확보

[추가] "📋 PRDAnalyzer 로딩 설정" 섹션
- 이유: 어떤 Agent가 이 문서를 로딩해야 하는지 명시 필요
- 효과: 문서-Agent 매핑 명확화

[핵심 변경] 키워드 기반 판별 섹션 (2.2)
- 기존: prd-analyzer.js에 QUANTITATIVE_KEYWORDS 배열 하드코딩
- TO-BE: 이 문서에서 키워드 정의, PRDAnalyzer가 문서에서 동적 추출
- 효과: 키워드 추가/수정이 문서 편집만으로 가능

[통합] 레퍼런스 자동 매핑 섹션 (4)
- 출처: PRD_REFERENCE_MAP.md 전체 내용
- 이유: PRD에 레퍼런스 없을 때 자동 제안 기능을 하나의 문서에서 제공
- 효과: PRDAnalyzer가 별도 문서 로딩 없이 레퍼런스 매핑 가능

[삭제 대상] 기존 3개 분리 문서
- PRD_TEMPLATE_V2.md → 이 문서로 통합
- PRD_TYPE_PIPELINE.md → 이 문서로 통합
- PRD_REFERENCE_MAP.md → 이 문서로 통합
