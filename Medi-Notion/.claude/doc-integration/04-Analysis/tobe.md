...
<!--
[통합 배경]
AnalysisAgent SPEC과 DB 접근 정책이 분리되어 있음.
현재 AnalysisAgent 자체가 미구현 상태.
통합 문서로 만들어 AnalysisAgent 구현 시 단일 참조 문서로 활용.
-->
...
<!-- [추가] AnalysisAgent가 이 문서를 로딩하도록 명시 -->
...
<!-- [기존 유지] ANALYSIS_AGENT_SPEC.md 섹션 1 -->
...
<!-- [기존 유지] ANALYSIS_AGENT_SPEC.md 섹션 2 흐름도 -->
...
<!-- [기존 유지] ANALYSIS_AGENT_SPEC.md 프롬프트 설계 섹션 -->
...
<!-- [기존 유지] ANALYSIS_AGENT_SPEC.md 섹션 4 -->
...
<!-- [기존 유지] ANALYSIS_AGENT_SPEC.md 섹션 5 -->
...
[통합] ANALYSIS_AGENT_SPEC.md + DB_ACCESS_POLICY.md → 단일 문서
- 이유: AnalysisAgent 설계와 DB 접근 정책이 분리되어 있어 구현 시 여러 문서 참조 필요
- 효과: AnalysisAgent 구현 시 단일 문서만 참조하면 됨

[추가] "📋 Agent 로딩 설정" 섹션
- 이유: AnalysisAgent가 어떤 문서를 로딩해야 하는지 명시 필요
- 효과: 문서-Agent 매핑 명확화

[추가] SubAgent도 DB 정책 섹션 로딩
- 이유: SubAgent도 쿼리 생성 시 DB 접근 정책 알아야 함
- 효과: 모든 Agent가 동일한 DB 정책 적용

[통합] DB 접근 정책 섹션 (섹션 3)
- 출처: DB_ACCESS_POLICY.md 전체 내용
- 이유: AnalysisAgent가 SQL 실행 시 계정/권한 정책 필요
- 효과: 단일 문서에서 분석 로직 + DB 정책 모두 확인 가능

[삭제 대상] 기존 2개 분리 문서
- ANALYSIS_AGENT_SPEC.md → 이 문서로 통합
- DB_ACCESS_POLICY.md → 이 문서로 통합
- 삭제 시점: AnalysisAgent 구현 완료 후

[구현 필요]
- AnalysisAgent 자체가 현재 미구현 상태
- 이 문서를 기반으로 orchestrator/agents/analysis-agent.js 구현 필요
-->
