...
> **문서 버전**: 2.1.0
> **최종 업데이트**: 2025-12-19
> **변경 이력**: 전체 파이프라인 플로우 다이어그램 추가 (HITL 포함)
> **상위 문서**: `CLAUDE.md`
> **대상 Agent**: Orchestrator, Leader, SubAgent, OutputValidator
...
│        │                 │                   │                   │                        │
│        ▼                 ▼                   ▼                   ▼                        │
│   IA.md            src/*.ts          PRD 체크리스트       PASS? ──┬── YES → 완료          │
│   Wireframe.md     tests/*.ts        6/6 매칭 확인              │                        │
│   SDD.md                             (3단계 매칭)                └── NO → 재시도 (최대 5회)│
│   HANDOFF.md                                                                             │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│  │ • Protected Path 거부 (.claude/global/*)                               │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
│  Layer 4: Audit & Integrity (Utils)                                         │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ • AuditLogger: 보안 이벤트 추적, 민감정보 마스킹                           │  │
│  │ • RulebookValidator: .claude/global/* SHA256 해시 검증                  │  │
...
| **RulebookValidator** | `.claude/global/*` 파일 SHA256 해시 검증 |
...
│   │   └── output-validator.js # 산출물 검증 (3단계 PRD 매칭)
│   │
