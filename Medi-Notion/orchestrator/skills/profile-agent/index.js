/**
 * ProfileAgent - 회원 프로필 분석 및 세그먼트 도출
 *
 * QueryAgent를 활용하여 세그먼트별 프로필 데이터를 분석
 *
 * @version 1.0.0
 * @since 2025-12-19
 */
...
import { QueryAgent } from '../query-agent/index.js';
...
export class ProfileAgent {
   ...
   */
  async initialize() {
    this.skill = await this.skillLoader.loadSkill('profile-agent');
    await this.queryAgent.initialize();
    console.log('[ProfileAgent] Initialized with SKILL.md');
    return this;
  }
  ...
  async analyzeProfiles(input) {
    ...
    console.log('[ProfileAgent] Starting profile analysis');
    ...
    for (const segmentKey of segments) {
      ...
      try {
        ...
        console.error(`[ProfileAgent] Segment ${segmentKey} analysis failed: ${error.message}`);
  ...
  async _analyzeSegment(segmentKey, segmentDef, timeRange) {
    ...
    const result = await this.queryAgent.executeQuery(sql, `Segment: ${segmentDef.name}`);
  ...
  async _analyzeDemographics(timeRange) {
    ...
    const result = await this.queryAgent.executeQuery(sql, 'Demographics Analysis');
    return result.data;
  }
  ...
  async _analyzeBehaviors(timeRange) {
    ...
    const result = await this.queryAgent.executeQuery(sql, 'Behavior Analysis');
    return result.data;
  }
